...
INITIALIZATION.
  ...
  t_trkorr = 'Transport request if the target package is transportable (otherwise empty)'(t19).
  ...
  t_intf_3 = 'Create interface pool as copy of ZIF_SOAR_MANAGER_TEST'(t26).
...
START-OF-SELECTION.
  TRY.
    ...
    CATCH cx_root INTO DATA(error).
      CALL METHOD ('LCL_APP')=>('DISPLAY_ERROR')
...
CLASS lcl_app IMPLEMENTATION.
  METHOD display_error.
    DATA(error_2) = error.
    DATA(messages) = VALUE string_table( ).
    WHILE error_2 IS BOUND.
      INSERT error_2->get_text( ) INTO messages INDEX 1.
      error_2 = error_2->previous.
    ENDWHILE.
    DATA(display_like_2) = COND #( WHEN display_like IS INITIAL THEN type ELSE display_like ).
    MESSAGE concat_lines_of( sep   = `. `
                             table = messages ) TYPE type DISPLAY LIKE display_like_2.
  ENDMETHOD.
